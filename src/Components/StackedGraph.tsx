...
import { dataObj, curveArr, ApexDatetime, dataNumArr, dateObj, ECO2Arr } from "../types";
...
export default function StackedGraph(props: { parameters: curveArr }) {
  ...
  const fetch = (date: string, name: string) => {
    const url =
  ...
  useEffect(() => {
    parameterData.map((item: dateObj) => {
      fetch(new Date(item.date).toISOString().split("T")[0], item.name);
    });
  }, []);
  function sortData(data: ECO2Arr) {
    if (data) {
      const mapCO2 = () =>
        data.map((item, i) => {
          if (item.ECO2 === 0 && i > 0) {
